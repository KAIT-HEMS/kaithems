...
ドメイン名 sh-center.org で運用しているセンターのHPは、これまでテレコンサービス（松澤さん）がWordPressを利用してサイトを制作し、Drive Networkでホスティングしていた。2025.12.01にDrive Networkの事業廃止に伴い、サーバーはDrive.LNXに移管された。センターの今後の運営を考慮し、静的サイトジェネレーターのHUGOを利用して新HPを制作し、さくらインターネットでホスティングする予定である。
本ドキュメントは、HUGOを利用して新HPコンテンツを追加し、アップロードする手順を記述する。
現時点で sh-center.orgのさくらインターネットへのドメイン移管は完了していないため、新HPは
さくらインターネットで https://sh-center.sakura.ne.jp としてホスティングしている。
